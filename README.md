...
Note: the `docker-compose.yml` at the repository root starts a MariaDB container. The backend now runs on PostgreSQL (via Supabase), so that file is currently out of date and does not provide a working local database — connect `DATABASE_URL` to your own PostgreSQL instance instead.
